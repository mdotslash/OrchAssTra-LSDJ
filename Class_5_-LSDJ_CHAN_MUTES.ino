// Sends a midi note when you push buttons that mutes one of four LSDJ channels
...
//LSDJ Keyboard Mode Mappings

int xPu1 = 36; // Mute Pu1 Off/On by sending C1
int xPu2 = 37; // Mute Pu2 Off/On by sending C#1
int xWav = 38; // Mute Wav Off/On by sending D-1 //
int xNoi = 39; // Mute Noise Off/On by D#1 //
...
void loop() {
  ...
  if (left_button.fell()) { //much simpler way of saying  prev_button == 1 && button == 0
    usbMIDI.sendNoteOn(xPu1, 127, 16);// send MIDI mute on Channel 1// drum1.noteOn(); //only happens once and does not need a corresponding note off
    Serial.println("left on");
  }
  if (middle_button.fell()) {
    //  envelope1.noteOn(); //only happens once
    usbMIDI.sendNoteOn(xPu2, 127, 16);
    Serial.println("middle on");
  }
  if (right_button.fell()) {
    //  envelope1.noteOn(); //only happens once
    usbMIDI.sendNoteOn(xWav, 127, 16);
    Serial.println("right on");
  }

  // Note OFF messages when each button is RELEASED

  if (left_button.rose()) { //much simpler way of saying  prev_button == 1 && button == 0
    usbMIDI.sendNoteOff(xPu1, 0, 16);// send MIDI mute on Channel 1// drum1.noteOn(); //only happens once and does not need a corresponding note off
    Serial.println("left off");
  }
  if (middle_button.rose()) {
    //  envelope1.noteOn(); //only happens once
    usbMIDI.sendNoteOff(xPu2, 0, 16);
    Serial.println("mid off");
  }
  if (right_button.rose()) {
    //  envelope1.noteOn(); //only happens once
    usbMIDI.sendNoteOff(xWav, 0, 16);
    ...
    for ( int x_count = 0; x_count < 8; x_count++) {
      for ( int y_count = 0; y_count < 8; y_count++) {
        ...
        if (left_button.read() == 0) { //if the button is down
          if (bitmap[xy_count] > 0 && y_count < 4) { //is the bitmap >0 at this LED and are we in a column less than 4?
        ...
        if (right_button.read() == 0) {
          if (bitmap[xy_count] > 0 && x_count > 4) {
            //set_pixel_HSV(led to change, hue,saturation,value aka brightness)
            float r1 = random(100) / 400.0;
            float color1 = bitmap[xy_count] / 8.0;
            set_pixel_HSV(xy_count, .8 + color1 , 1 - r1 , .5 - r1);
          }
        }
      }
    }
